...
| P4 | Remove glucose → starvation: Medium, Glucose "None"; `runTicks(30)`; narrator key is `starve.nosugar`; ATP gauge word is "very low"; `runTicks(120)`; growth reads "not growing"; the cell view shows ADP glyphs ≥ 15 and ATP glyphs ≤ 1 |
...
| Plot overlay | `plot.setOverlay(points, style)` and `plot.setInputMode('sketch', onDone)`, both implemented as no-ops that throw "not in M1" | 1.2 predicted protein curve, scored against the Recorder by replay |
